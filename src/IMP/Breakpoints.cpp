...
void initBreakpoints(const std::vector<std::shared_ptr<AlignmentRecord>>& alns,
	...
	for (auto bp : speciesBounds)
		result.push_back(Breakpoint(bp, fixedWeight));
	for (auto aln : alns) {
		result.push_back(Breakpoint(aln->tStart, 1));
		result.push_back(Breakpoint(aln->tEnd, 1));
	}
...
void createWaste(const std::vector<Breakpoint>& breakpoints, unsigned int minLength,
	...
	for (size_t i = 1; i < breakpoints.size(); i++) {
		...
		if (distance <= minLength) { // too close for atom to be in between
			...
			prev->bpPositions.insert(breakpoints[i].position);
